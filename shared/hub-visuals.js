...
(function () {
  ...
  const V = {
    ...
    // Компетенції PM: шість областей однією сіткою, підсвічена одна
    '26-competencies.html': S(
      [0,1,2].map(i => [0,1].map(j =>
        r(18 + i*96, 24 + j*46, 84, 34, (i===1&&j===0) ? 'acc-box' : 'o', 5)).join('')).join('') +
      [0,1,2].map(i => [0,1].map(j =>
        r(26 + i*96, 33 + j*46, 44, 5, (i===1&&j===0) ? 'acc' : 'f-rule', 2) +
        r(26 + i*96, 43 + j*46, 62, 4, 'f-s', 2)).join('')).join('')),
    // Лекція з ризиків: матриця P×I 5×5 із зонами, одна клітинка — поточний ризик
    '27-risk-lecture.html': S(
      [0,1,2,3,4].map(i => [0,1,2,3,4].map(j => {
        const sc = (5 - i) * (j + 1);
        const c = (i === 1 && j === 3) ? 'acc-box' : sc >= 15 ? 'f-clay' : sc >= 8 ? 'f-s' : 'o';
        return r(78 + j*30, 10 + i*20, 26, 16, c, 3);
      }).join('')).join('') +
      t(18, 58, 'P × I', 'acc-t')),
